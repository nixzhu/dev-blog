...
我们之所以用缓存，自然是期望缓存带来性能提升，这就要求用“键”访问缓存得到“值”的速度非常快。不过受限于系统资源，NSCache 会自动管理缓存里的内容（通常是移除一些键值），这就和字典不一样了。

我们使用 NSCache 的方式就是往里面添加、移除以及查询“键值对”。
...
public func setObject(obj: AnyObject, forKey key: AnyObject, cost g: Int) {
    ...
    if let entry = _entries[keyRef] {
        ...
        if entry.cost != g {
            ...
            insert(entry) // 由此可见，所有的 entry 会组成一个链表，以 cost 排序（这要建立在每个 entry 的 cost 不相同的前提下）
        }
    } else {
...
以 _byCost 所代表的 entry 开始，寻找第一个“价值”大于本次将插入的 entry 的元素，找到了就好放置 entry 了，设置好它的 prevByCost 和 nextByCost。注意这里并没有修改 e 的 prevByCost，这说明价值越小的排越前。但似乎链表也被破坏了。

但平常我们使用 NSCache 时，并不管“价值”，上面的 insert 其实不会被调用。这说明，链表只在我们很关心 entry 的价值时才会建立起来，且 _byCost 指向当前价值最小的一个，便于实现删除逻辑。

以上是个人的初浅理解，如有错漏，欢迎指正！
